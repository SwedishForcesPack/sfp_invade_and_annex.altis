...
// #define KEY 0x0F // TAB // http://community.bistudio.com/wiki/DIK_KeyCodes
...
GVAR(menu_giveLeaderShip) = {
	...
	[] call GVAR(actions_remove);
	{
		PUSH_START(GVAR(actions_ids))
			player addAction [
				format["<t color='#FF75F9'><img image='\A3\ui_f\data\gui\Rsc\RscDisplayConfigViewer\bookmark_gs.paa' size='0.7' /> Give leadership to %1</t>", name _x],
...
GVAR(menu_squadOptions) = {
	...
	_join = [group player] call GVAR(options_getJoin);
	{
		PUSH_START(GVAR(actions_ids))
			player addAction [
				format["<t color='#FF75F9'>%1 %2</t>", _x, if(_join==_forEachIndex) then {"(Current)"} else {""}],
	...
	_accept = [group player] call GVAR(options_getAccept);  		
	{
		PUSH_START(GVAR(actions_ids))
			player addAction [
				format["<t color='#6FFFD4'>%1 %2</t>", _x, if(_accept==_forEachIndex) then {"(Current)"} else {""}],
...
GVAR(menu_main) = {
	...
	_myJoin = [group player] call GVAR(options_getJoin);  			
  	{
  		if(side _x == side player && group _x != group player && isPlayer _x && alive _x && !(vehicle _x in allUnitsUav)) then {
...
"Server: squad manager is active, hold 'T' and use your mousewheel to bring it up" call GVAR(msg);
