...
private ["_bastion","_CHgroupSize","_CHGroups","_SVehGroups","_AVgroupSize","_AVehGroups","_LVehGroups","_LVgroupSize","_PAgroupSize","_PApatrols","_JIPmkr","_HPpatrols","_HPgroupSize"];
VictoryColor="colorGreen";
hostileColor="colorRed";
bastionColor="colorBlue";
...
switch (_AVgroupSize) do
{
	case 0:{_AVgroupSize=[0,0]};
	case 1:{_AVgroupSize=[2,4]};
	case 2:{_AVgroupSize=[4,8];};
	case 3:{_AVgroupSize=[8,12];};
	case 4:{_AVgroupSize=[12,16];};
	case 5:{_AVgroupSize=[16,20];};
};
switch (_CHgroupSize) do
{
	case 0:{_CHgroupSize=[0,0]};
...
};
{
	...
	if (isnil "_eosMarkers") then {_eosMarkers=[];};
		...
		null = [_x,[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups,_AVgroupSize],[_CHGroups,_CHgroupSize],_settings] execVM "eos\bastion\b_core.sqf";;
}foreach _JIPmkr;