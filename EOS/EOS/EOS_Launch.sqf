...
_SVehGroups=_SVeh select 0;
_CHGroups=_SVeh select 1;
_CHgroupSize=_SVeh select 2;
...
};
{
	...
	if (isnil "_eosMarkers") then {_eosMarkers=[];};
		...
		null = [_x,[_HPpatrols,_HPgroupSize],[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups,_AVgroupSize],[_SVehGroups,_CHGroups,_CHgroupSize],_settings] execVM "eos\EOS\EOS_Core.sqf";
}foreach _JIPmkr;