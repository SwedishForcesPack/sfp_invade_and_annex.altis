...
waituntil {triggeractivated _bastTaken};
	...
	_aGroup=[];
		...
		_n=_n+1;
						...
						_pos = [_pos,0,20,3,0,30,0] call BIS_fnc_findSafePos;
...
//SPAWN ARMOURED VEHICLES
	_AV=[];
		while {(count _AV) < _AVehGroups} do
		{
					...
					_cGroup=[_pos,_faction,_side] call EOS_Armour;
					_AV=_AV+[_cGroup];
					sleep 0.8;
				if (_debug) then {
	...
	_TR=[];
		...
		while {_n < _CHGroups} do
		{
			...
			0=[_mkr,_CHgroupSize,_faction,_CHType,_n,_bastActive,_side] execVM "eos\core\Spawn_Aircraft.sqf";
			_TR=_TR+[_n];
