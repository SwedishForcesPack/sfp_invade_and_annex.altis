...
private ["_radius","_pos","_side","_AVGrp","_bGroup","_bastion","_eType","_civZone","_heightLimit","_AT","_ATCHGroups","_eGrpB","_eSize","_eGrpsB","_eGrp","_eGrps","_d","_dGrps","_aMin","_aSize","_aGrps","_a","_aGrp","_bMin","_units","_bSize","_bGrps","_b","_bGrp","_trig","_cache","_grp","_crew","_vehicle","_actCond","_mAN","_mAH","_distance","_mA","_settings","_cGrp","_grpID","_LVvehGrpCache","_cSize","_cGrps","_taken","_clear","_strFac","_faction","_veh","_n","_c","_eosAct","_eosActivated","_debug","_mkr","_mPos","_mkrX","_mkrY"];
...
_d=(_this select 4);_dGrps=_d select 0;_eGrps=_d select 1;_eGrpsB=_d select 2;_eSize=_d select 3;
		...
		if (_cache) then {
				...
				_units=_eosActivated getvariable _cacheGrp;
					...
					if (_debug)then{player sidechat format ["ID:%1,restore - %2",_cacheGrp,_units];};
							...
							if (_bMin > 0) then {
								_pos= [_mPos,_radius] call aw_findPos;
									_bGroup=[_pos,_bSize,_faction,_side] call EOS_spawnPatrol;
										0 = [_bGroup,_radius,_mPos] call EOS_Patrol;
	...
	_radius=_mkrX * 0.7;
		while {(count _cGrp) < _cGrps} do
		{
				_pos= [_mPos,_radius] call aw_findPos;
					...
					_cGrp=_cGrp+[_cGroup];
						0 = [(_cGroup select 2),_radius,_mPos] call EOS_Patrol;
						sleep 0.1;
...
//SPAWN ARMOURED VEHICLES
	...
	_radius=_mkrX * 0.4;
		...
		{
			_pos= [_mPos,_radius] call aw_findPos;
					_dGroup=[_pos,_faction,_side] call EOS_Armour;
					0 = [(_dGroup select 2),_radius,_mPos] call EOS_Patrol;
	...
	_eGrpB=[];
		while {_n < _eGrpsB} do
		{
			...
			_eType=true;_eGrpsB=0;
						...
						0=[_mkr,_eSize,_faction,_eType,_n,_eosActivated,_side] execVM "eos\core\Spawn_Aircraft.sqf";
						_eGrpB=_eGrpB+[_n];
