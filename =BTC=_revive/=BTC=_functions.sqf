...
BTC_get_gear =
{
	...
	_array_bullet = [];
	{
		...
		_brack = 0;
		{if (_x == 40) then {_brack = _brack + 1;};} foreach _array_mag;
		...
		for "_i" from 0 to (count _array_mag - 1) do
		{
			...
			if !(_cond_class) then
			{
				_array_class_x = _array_class_x + [_n];//diag_log text format ["X %1",_array_class_x];
			}
			else
			{
...
BTC_drag =
{
	...
	{
		//fix respawn exploit while being dragged
		_key     = _this select 1;
		switch _key do
		{
			case 28:
