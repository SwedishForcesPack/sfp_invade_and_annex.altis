...
if(((_unit isKindOf "Helicopter") OR (_type in _slingable)) AND !(_type in _notSlingable)) then {[_unit] execVM "scripts\aw_sling\aw_sling_setupUnit.sqf"};

if(((_unit isKindOf "Ship") OR (_type in _pushable)) AND !(_type in _notPushable)) then {[_unit] execVM "scripts\aw_boatPush\aw_boatPush_setupUnit.sqf"};

if(_type in _ghosthawk) then {
	[_unit] execVM "scripts\heliDoor.sqf";
...
_unit addAction ["<t color='#3f3fff'>Clear Inventory</t>","scripts\clear.sqf",[],-97,false];
