...
if ((_playerVehicle == _heli) && (_playerAssignedRole == "Cargo")) then {
	if ((_heli getVariable "getOutActionExist") == 0) then {
		...
		_id = player addAction [_action, "scripts\usableCargoRamp\getOut.sqf", [_heli], 6.15, true, true];
		_heli setVariable ["getOutId", _id, false];
		_heli setVariable ["getOutActionExist", 1, false];
	};
} else {
	if ((_heli getVariable "getInActionExist" == 0) && ((_heli emptyPositions "Cargo") != 0)) then { // && vehicle player == player
