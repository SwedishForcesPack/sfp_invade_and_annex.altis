								...
								if (_aMin > 0) then {
										...
										if (!surfaceiswater _mPos) then {
											0=[_mPos,units _aGroup,_mkrX,0,[0,20],true] call callHouseScript;
												...
												0 = [_aGroup,_mkr] call EOS_Patrol;
													};
					...
					if (_debug)then{player sidechat format ["ID:%1,restore - %2",_cacheGrp,_units];};
							...
							if (_bMin > 0) then {	
									...
									_bGroup=[_pos,_bSize,_faction,_side] call EOS_spawnPatrol;
										0 = [_bGroup,_mkr] call EOS_Patrol;
...
//SPAWN LIGHT VEHICLES
	...
	if (isnil "_cGrp") then {_cGrp=[];};	
				_pos = [_mkr,true] call SHK_pos;
					...
					0 = [(_cGroup select 2),_mkr] call EOS_Patrol;
...
//SPAWN ARMOURED VEHICLES
	...
	if (isnil "_dGrp") then {_dGrp=[];};
			_pos = [_mkr,true] call SHK_pos;
					...
					0 = [(_dGroup select 2),_mkr] call EOS_Patrol;
...
//SPAWN STATIC PLACEMENTS
	for "_counter" from 1 to _eGrps do {
		...
		if (isnil "_eGrp") then {_eGrp=[];};
				_pos = [_mkr,true] call SHK_pos;
				_eGroup=[_pos,_faction,_side] call EOS_spawnStatic;
...
//SPAWN CHOPPER
	...
	if (isnil "_fGrp") then {_fGrp=[];};	
						_fGroup=[(markerpos _mkr),_fSize,_faction,_side] call EOS_Chopper;	
