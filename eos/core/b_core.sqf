...
private ["_CHside","_mkrAgl","_initialLaunch","_pause","_eosZone","_hints","_waves","_aGroup","_side","_actCond","_enemyFaction","_mAH","_mAN","_distance","_grp","_cGroup","_bGroup","_CHType","_time","_timeout","_faction"];
...
_SVeh=(_this select 4);_CHGroups=_SVeh select 0;_CHgroupSize=_SVeh select 1;
...
//SPAWN LIGHT VEHICLES		
	...
	for "_counter" from 1 to _LVehGroups do {		
				_pos = [_mPos, (_Placement +200), random 360] call BIS_fnc_relPos;	
					_grp=[_pos,_PAgroupSize,_faction,_side] call EOS_LightVeh;
					_bGroup set [count _bGroup,_grp];		
					if (_debug) then {player sidechat format ["Light Vehicle:%1 - r%2",_counter,_LVehGroups];0= [_mkr,_counter,"Light Veh",(getpos leader (_grp select 2))] call EOS_debug};
		};	
...
//SPAWN ARMOURED VEHICLES
	...
	for "_counter" from 1 to _AVehGroups do {	
				_pos = [_mPos, _Placement, random 360] call BIS_fnc_relPos;
					_grp=[_pos,_faction,_side] call EOS_Armour;
					_cGroup set [count _cGroup,_grp];					
					if (_debug) then {player sidechat format ["Armoured:%1 - r%2",_counter,_AVehGroups];0= [_mkr,_counter,"Armour",(getpos leader (_grp select 2))] call EOS_debug};
		};
...
//SPAWN HELICOPTERS		
	...
	for "_counter" from 1 to _CHGroups do {	
						_CHside=_side;
						_eGroupB=[(markerpos _mkr),_CHgroupSize,_faction,_CHside] call EOS_Chopper;
						_dGroup set [count _dGroup,_eGroupB];
					if ((_CHgroupSize select 0) > 0) then {
						null = [_mkr,_eGroupB,_counter] execvm "eos\functions\TransportUnload_fnc.sqf";
												}else{
						_wp1 = _grp addWaypoint [(markerpos _mkr), 0];  
						_wp1 setWaypointSpeed "FULL";  
						_wp1 setWaypointType "SAD";};
							if (_debug) then {player sidechat format ["Chopper:%1",_counter];0= [_mkr,_counter,"Chopper",(getpos leader (_eGroupB select 2))] call EOS_debug};
			};	
	...
	sleep 1;
			...
			{			
					...
					_mkr setmarkeralpha _mAN;
						...
						if (_eosZone) then {
null = [_mkr,[_PApatrols,_PAgroupSize],[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups,0,0,0],[_faction,_mA,250,_CHside]] execVM "eos\core\EOS_Core.sqf";
...
null = [_mkr,[_PApatrols,_PAgroupSize],[_LVehGroups,_LVgroupSize],[_AVehGroups],[_CHGroups,_CHgroupSize],_settings,[_pause,_waves,_timeout,_eosZone,_hints],true] execVM "eos\core\b_core.sqf";
						};};
