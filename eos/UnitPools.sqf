private ["_sqdPool","_unit","_faction","_InfPool","_tempArray","_type","_diverPool","_ArmPool","_MotPool","_ACHPool","_CHPool","_uavPool","_shipPool","_stPool"];
...
////////////////////////////////////////////////////////////////////////////////////////
	...
	_sqdPool=[];
		...
		for "_i" from 0 to 5 do{_unit=_InfPool select (floor(random(count _InfPool)));
		_sqdPool set [count _sqdPool,_unit];};
		_tempArray=_sqdPool;
						};
...
if (_type==1) then {_tempArray=_diverPool};
	...
	if (_type==2) then {
		...
		for "_i" from 0 to 4 do{
			...
			_temparray set [count _temparray,_sqdPool];
						};
...
if (_type==4) then {_tempArray=_ArmPool select (floor(random(count _ArmPool)));};
if (_type==5) then {_tempArray=_ACHPool select (floor(random(count _ACHPool)));};
...
if (_type==7) then {_tempArray=_stPool select (floor(random(count _stPool)));};
if (_type==8) then {_tempArray=_uavPool select (floor(random(count _uavPool)));};
if (_type==9) then {_tempArray=_shipPool select (floor(random(count _shipPool)));};
