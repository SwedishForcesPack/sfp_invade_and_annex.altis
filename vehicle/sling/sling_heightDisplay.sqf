...
while{(_helo getVariable "sling_attached") AND (alive _helo)} do
{
	waitUntil{sleep 0.1;(player == driver _helo)};
	while{(_helo getVariable "sling_attached") AND (player == driver _helo)} do
	{
		...
		_string = format["Load Height: %1",round _height];
