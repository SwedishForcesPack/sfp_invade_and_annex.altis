/*
Authors: Rarek. Jester, Razgriz33, Kamaradski [AhoyWorld.co.uk]
Contributors: Quiksilver, chucky (allfps.com.au)
Editors: yournamehere
*/

//Create base array of differing side missions

private ["_firstRun","_mission","_isGroup","_obj","_skipTimer","_awayFromBase","_road","_position","_briefing","_altPosition","_flatPos","_accepted","_randomPos","_spawnGroup","_unitsArray","_randomDir","_hangar","_x","_sideMissions","_completeText","_roadList","_armour","_armourGroup","_car","_carGroup","_truck"];
_sideMissions = 
...
case "destroyChopper":
		{
			...
			for "_x" from 0 to 1 do
			{
				...
				[(units _spawnGroup)] call QS_setSkill3;
			...
			[_armourGroup, _flatPos, 300] call BIS_fnc_taskPatrol;
			_unitsArray = _unitsArray + [_armourGroup];
			[(units _armourGroup)] call QS_setSkill4;
			_armour lock 3;
...
case "destroySmallRadar":
		{
			...
			for "_x" from 0 to 1 do
			{
				...
				[_spawnGroup, _flatPos, 300] call BIS_fnc_taskPatrol;
				[(units _spawnGroup)] call QS_setSkill2;
			...
			[_armourGroup, _flatPos, 300] call BIS_fnc_taskPatrol;
			_unitsArray = _unitsArray + [_armourGroup];
			[(units _armourGroup)] call QS_setSkill1;
			_armour lock 3;
...
case "destroyExplosivesCoast":
		{
			...
			_armour = "O_Boat_Armed_01_hmg_F" createVehicle _randomPos;
			waitUntil{!isNull _armour};

				"O_diver_F" createUnit [_randomPos,_armourGroup];
				"O_diver_F" createUnit [_randomPos,_armourGroup];
				"O_diver_TL_F" createUnit [_randomPos,_armourGroup];
...
case "destroyResearchOutpost":
		{
			...
			_carGroup = createGroup east;
			_randomPos = [_flatPos, 10, 20, 5, 0, 1, 0] call BIS_fnc_findSafePos;
			_car = "C_SUV_01_F" createVehicle _randomPos;
			waitUntil{!isNull _car};

				"O_Soldier_SL_F" createUnit [_randomPos,_carGroup];
				"O_officer_F" createUnit [_randomPos,_carGroup];
				"O_soldierU_medic_F" createUnit [_randomPos,_carGroup];
				"O_soldier_repair_F" createUnit [_randomPos,_carGroup];
				
				((units _carGroup) select 0) assignAsDriver _car;
				((units _carGroup) select 1) assignAsCargo _car;
				((units _carGroup) select 2) assignAsCargo _car;
				((units _carGroup) select 3) assignAsCargo _car;
				((units _carGroup) select 0) moveInDriver _car;
				((units _carGroup) select 1) moveInCargo _car;
				((units _carGroup) select 2) moveInCargo _car;
				((units _carGroup) select 3) moveInCargo _car;
			
			_unitsArray = _unitsArray + [_carGroup];
			[(units _carGroup)] call QS_setSkill2;
			_car lock 3;
...
case "destroyWeaponsSupply":
		{
			...
			[_armourGroup, _flatPos, 300] call BIS_fnc_taskPatrol;
			_unitsArray = _unitsArray + [_armourGroup];
			[(units _armourGroup)] call QS_setSkill3;
			_armour lock 3;
