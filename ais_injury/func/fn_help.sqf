...
_animChangeEVH = _helper addEventhandler ["AnimChanged", {
	...
	if (primaryWeapon _helper != "") then {
		if (time >= tcb_animDelay) then {tcb_helperStopped = true; Hint "Stopped healing because your weapon has been raised";};
	} else {
...
if (!alive _injuredperson) exitWith {["He's dead, Jim.",0, 0.035 * safezoneH + safezoneY,5,0.3] spawn BIS_fnc_dynamicText};
