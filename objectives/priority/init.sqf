/***********************************************
|
|	TITLE: Priority Target Template
|	LOCATION: objectives/priority/init.sqf
|	AUTHOR: Rarek [AW]
|	
|	DESCRIPTION
|		|	Initialises a Priority Target using this
...
private ["_priorityObjs", "_genericObjs", "_priorityTargets", "_missionDetails", "_spawnedObjects", "_title", "_briefObj", "_successMsg", "_failureMsg", "_posConditions", "_enemies", "_PT_Create", "_PT_Success", "_PT_Failure", "_enemiesArray"];

/***********************************************
|
|	You should only edit this part of the template
|	to include more Priority Targets in I&A.
|
...
_priorityObjs = [];
_enemiesArray = [];
_randomPriorityIndex = _priorityTargets select (round(random ((count _priorityTargets) - 1)));
call compile preProcessFileLineNumbers format["objectives/priority/missions/%1.sqf", _randomPriorityIndex];
...
switch (_posType) do
{
	case "land":
	{
		while {!_isGoodPos} do
		{
			_isGoodPos = true; _priorityPos = [];
			while {(count _priorityPos) < 1} do
			{
				_randomPos = [] call BIS_fnc_randomPos;
				_priorityPos = _randomPos isFlatEmpty [5, 1, 0.5, 10, 0, false];
			};
			_isGoodPos = [_priorityPos] call AW_fnc_distanceCheck;
		};
	};
	...
	case "custom":
	{
		_priorityPos = [] call _SM_Pos;
	};
};

_spawnedObjects = [_priorityPos] call _SM_Create;
_priorityObjs = _spawnedObjects select 0;
...

{
	...
	{
		_randomPos = [[[_priorityPos, _groupSpawnDistance]],["water","out"]] call BIS_fnc_randomPos;
...
_fuzzyPos =
[
	((_priorityPos select 0) - 300) + (random 600),
	((_priorityPos select 1) - 300) + (random 600),
	0
];

{ _x setMarkerPos _fuzzyPos; } forEach ["priorityMarker", "priorityCircle"];
"priorityMarker" setMarkerText format["priority Target: %1", _title]; publicVariable "priorityMarker";
priorityMarkerText = _title; publicVariable "priorityMarkerText";

priorityTargetUp = true; publicVariable "priorityTargetUp";
showNotification = ["NewPriorityTarget", _briefObj]; publicVariable "showNotification";

/*---------------------------------------------------------------------------
This is the key. _outcome can be used to get either a TRUE or FALSE return
from the _SM_Run function. Have a WHILE loop or a WAITUNTIL in that function
to properly simulate the workings of any mission.

This allows for Side Missions / anything with multiple stages as well as
initial objectives.

TRUE = Mission Success
FALSE = Mission Failure
---------------------------------------------------------------------------*/
_outcome = [_priorityObjs] call _SM_Run;
...
[] spawn { [_priorityObjs] call AW_fnc_deleteObjects; };
